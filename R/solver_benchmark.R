...
#'   graph size, factorization mode, setup time, solve time, and total time.
#'
#' @examples
#' library(terra)
#' data(melip)
                                       ...
                                       perm = TRUE)
{
  ...
  rows <- vector("list", length(factorization) * n_replicates)
  row_id <- 0L
  ...
  for (mode in factorization)
  {
    for (replicate in seq_len(n_replicates))
    {
      ...
      solver_state <- .terradish_solver_setup(
        ...
        conductance,
        solver = "direct",
        solver_control = list(factorization = mode, perm = isTRUE(perm))
      )
