...
#' @param measurement_model Measurement model used by optimizer probes.
#' @param nu Optional number of genetic markers passed to measurement models
#'   that require it.
#' @param theta Optional starting values for conductance parameters.
#' @param nonnegative Should regression-like measurement models constrain the
#'   slope to be nonnegative during probes?
#' @param optimizer_probe If \code{TRUE}, run short optimization probes.
#' @param solver_probe If \code{TRUE}, benchmark direct solver settings with
#'   \code{\link{terradish_solver_benchmark}}.
#' @param coarse_probe If \code{TRUE}, include a coarse-raster warm-start probe
#'   when \code{data} retains a continuous raster stack.
#' @param optimizer_candidates Character vector of optimizer probes. Supported
#'   values are \code{"newton_hager_zhang"}, \code{"bfgs_hager_zhang"}, and
#'   \code{"bfgs_armijo"}.
#' @param probe_maxit Maximum optimizer steps for each optimization probe. Keep
#'   this small; the assessment is meant to compare early evaluation cost, not
#'   replace a final model fit.
#' @param factorization,solve_backend Direct solver settings to benchmark when
#'   \code{solver_probe = TRUE}.
#' @param coarse_factor Aggregation factor used for the optional coarse-raster
#'   probe.
#' @param coarse_refine_maxit Maximum full-resolution refinement steps for the
#'   optional coarse-raster probe.
#' @param loglik_tolerance Maximum log-likelihood difference allowed when
#'   choosing the fastest acceptable optimization probe.
#' @param max_seconds Soft time budget. The current probe is allowed to finish,
#'   but later probes are skipped once this budget is exceeded.
#' @param cores Number of worker processes passed to optimization probes.
#' @param verbose Should assessment progress be printed?
#'
#' @details
#' The assessment is advisory. It does not change package defaults and should
#' not be treated as a formal model-selection procedure. Short probes are useful
#' for detecting expensive line searches, poor direct-solver choices, or cases
#' where BFGS avoids costly repeated Hessian solves. Final model interpretation
#' should still be based on a normal \code{\link{terradish}} fit using the
#' chosen settings.
#'
#' The returned \code{$recommended} component is intentionally a plain list of
#' arguments. Users can inspect or edit it before passing the pieces into
#' \code{\link{terradish}}.
#'
#' @return An object of class \code{terradish_setting_assessment} with graph
#'   profile information, benchmark tables, recommended settings, and notes.
#'
#' @examples
#' \dontrun{
#' library(terra)
